...
*/
	...
	jsr	tbuf_init	/* init type-ahead buffer (cold boot only) */
...
/* Warm boot entry: bdosinit + disk select, preserves type-ahead buffer */
ccpwboot:
	lea	stack,%sp
	jsr	bdosinit	/* reset disk system, TPA, exc vectors */
	move.w  #32,%d0
	clr.l	%d1
	move.b  dskuser,%d1
	trap	#2		/* set user number */
	clr.l	%d0
	move.w  #14,%d0
	clr.l	%d1
	move.w	dskuser,%d1
