	...
	{				/*				    */
	  ...
	  {				/************************************/
	    *GBL.insptr++ = ch;		/* Yes, insert the character in buff*/
	    if(GBL.insptr >= &GBL.t_buff[TBUFSIZ])
		GBL.insptr = &GBL.t_buff[0]; /* Wrap insertion pointer    */
	     GBL.kbchar++;		/* Up count			    */
	  }				/************************************/
			...
			/* For internal use only	*/
{
	...
	if(GBL.remptr >= &GBL.t_buff[TBUFSIZ])
		GBL.remptr = &GBL.t_buff[0]; /* Wrap removal pointer */
