...
*********************************/
	...
	.globl  pgmld		|* this routine is public
...
*/
pgmld:
	movem.l	%d1-%a6, -(%sp)  | * save everything, just to be safe
	...
	beq	noreloc
	tst.w	rlbflg
	beq	doreloc			|* rlbflg=0: has reloc bits, use normal reloc
	bsr	brute_reloc		|* rlbflg!=0: no reloc data, scan and fix addresses
	bra	noreloc
doreloc:
	bsr	reloc			|* normal relocation using file's reloc bits
noreloc:
	tst	%d0
	bne	lddone
...
* Set up load addresses for cseg, dseg, bss, basepg, and stack
	...
	move.l	%d7,(%a2)
set0:	cmpi.w	#0x601b,magic
	beq	seta		|* 601b: always use tstart (absolute addresses)
	bra	set1		|* 601a: always load at LoAdr, reloc handles rest
seta:	move.l	tstart,cseg	|* 601b: cseg starts at tstart
	bra	set2
set1:	btst	#0,Flags(%a0)
	bne	sldhi
* relocatable, load low
	move.l	LoAdr(%a0),%d7
	add.l	#0x101,%d7	|* leave room for base page
	bclr	#0,%d7
...
rdtxt1:	move.w	(%a5)+,(%a6)+	|* move header sector to base page
	...
	cmpi.w	#0x601a,magic	|* short header?
	beq	rdtxt2
	subq.w	#8,%d7
...
brute_reloc:
* Brute-force relocation for non-relocatable 0x601a files (rlbflg != 0).
* These files have no relocation bits in the file, but all absolute address
* references are long words in the range [tstart, tstart+csize+dsize+bsize).
* We scan the loaded code+data text and add (cseg - tstart) to each match.
* Preserves a0 (LPB pointer needed by setrtn).
	move.l	cseg,%d5
	sub.l	tstart,%d5	|* d5 = relocation offset
	beq	br_done		|* if zero, nothing to do

	move.l	tstart,%d3	|* d3 = address range low (inclusive)
	move.l	csize,%d4
	add.l	dsize,%d4
	add.l	bsize,%d4
	add.l	%d3,%d4		|* d4 = address range high (exclusive)

	movea.l	cseg,%a2	|* a2 = scan pointer
	move.l	csize,%d2
	add.l	dsize,%d2	|* d2 = total text size in bytes
	movea.l	%a2,%a3
	adda.l	%d2,%a3
	subq.l	#2,%a3		|* a3 = last valid position for long word read

br_loop:
	cmpa.l	%a3,%a2
	bhi	br_done		|* past end of scannable text
	move.l	(%a2),%d6	|* read candidate long word
	cmp.l	%d3,%d6
	bcs	br_skip		|* below address range
	cmp.l	%d4,%d6
	bcc	br_skip		|* at or above address range high
	add.l	%d5,%d6		|* relocate: add offset
	move.l	%d6,(%a2)+	|* write back and advance past the long word
	bra	br_loop
br_skip:
	addq.l	#2,%a2		|* advance by one word
	bra	br_loop
br_done:
